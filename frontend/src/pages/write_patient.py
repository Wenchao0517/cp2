    ...
    <div style={{minHeight:'100vh',background:C.bg,fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",Roboto,sans-serif'}}>
      ...
      <nav style={{background:C.card,borderBottom:'1px solid '+C.border,padding:'0 32px',height:62,display:'flex',alignItems:'center',justifyContent:'space-between',position:'sticky',top:0,zIndex:100,boxShadow:'0 1px 0 '+C.border}}>
        ...
        <div style={{display:'flex',alignItems:'center',gap:10}}>
          ...
          <span style={{fontSize:13,fontWeight:600,color:C.text}}>{user?.full_name}</span>
          <button onClick={logout} style={{padding:'6px 14px',background:'transparent',color:C.muted,border:'1px solid '+C.border,borderRadius:7,fontSize:12,fontWeight:600,cursor:'pointer',marginLeft:4}}>Sign out</button>
        </div>
      </nav>
        ...
        {tab==='log' && (
          <div style={{maxWidth:460}}>
            <div style={{background:C.card,border:'1px solid '+C.border,borderRadius:18,padding:26}}>
              ...
              <p style={{fontSize:13,color:C.muted,marginBottom:24,margin:'0 0 22px'}}>Enter your blood glucose from your glucometer</p>
          ...
          <div style={{background:C.card,border:'1px solid '+C.border,borderRadius:18,overflow:'hidden'}}>
            ...
            {readings.length === 0 ? (
              <div style={{textAlign:'center',padding:'56px',color:C.muted}}>
                <div style={{color:C.border,marginBottom:10}}><IcoClip/></div>
