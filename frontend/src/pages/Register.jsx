...
const C = { green:'#00C48C', blue:'#3B82F6', border:'#E8EDF2', text:'#1A2332', muted:'#6B7A8F', red:'#FF6B6B' }
...
export default function Register() {
  ...
  return (
    <div style={{minHeight:'100vh',background:'linear-gradient(135deg,#F0FDF8 0%,#E8F4FD 100%)',display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:20,fontFamily:'-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif'}}>
      ...
      <div style={{background:'#fff',borderRadius:20,padding:'32px 36px',width:'100%',maxWidth:420,boxShadow:'0 4px 24px rgba(0,0,0,0.07),0 1px 4px rgba(0,0,0,0.04)'}}>
        <h2 style={{fontSize:18,fontWeight:700,color:C.text,margin:'0 0 6px',letterSpacing:'-0.3px'}}>Create your account</h2>
        <p style={{fontSize:13,color:C.muted,margin:'0 0 24px'}}>Start monitoring your diabetes risk today</p>
        ...
        <form onSubmit={handleSubmit}>
          <div style={{marginBottom:14}}>
            <label style={{display:'block',fontSize:12,fontWeight:700,color:C.muted,marginBottom:6,textTransform:'uppercase',letterSpacing:'0.6px'}}>Full Name</label>
            <div style={{position:'relative'}}>
              <span style={{position:'absolute',left:12,top:'50%',transform:'translateY(-50%)',color:C.muted}}><IcoUser/></span>
          ...
          <div style={{marginBottom:14}}>
            ...
            <div style={{position:'relative'}}>
              ...
              <input name="password" type="password" required value={form.password} onChange={handle} placeholder="Min 8 characters"
                style={{width:'100%',padding:'11px 12px 11px 36px',border:'1.5px solid '+C.border,borderRadius:10,fontSize:14,outline:'none',boxSizing:'border-box',fontFamily:'inherit',color:C.text}}/>
            </div>
          </div>

          <div style={{marginBottom:22}}>
            <label style={{display:'block',fontSize:12,fontWeight:700,color:C.muted,marginBottom:8,textTransform:'uppercase',letterSpacing:'0.6px'}}>I am a</label>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:8}}>
              {[['patient','Patient','🧑'],['doctor','Doctor / Clinician','👨‍⚕️']].map(([v,l,ico])=>(
                <button key={v} type="button" onClick={()=>setForm({...form,role:v})}
                  style={{padding:'10px 8px',border:'1.5px solid '+(form.role===v?(v==='patient'?C.green:C.blue):C.border),
                    borderRadius:10,background:form.role===v?(v==='patient'?'#F0FDF8':'#EFF6FF'):'transparent',
                    color:form.role===v?(v==='patient'?C.green:C.blue):C.muted,fontWeight:600,fontSize:13,cursor:'pointer',
          ...
          <button type="submit" disabled={loading}
            style={{width:'100%',padding:'12px',background:'linear-gradient(135deg,#00C48C,#00A070)',color:'#fff',border:'none',borderRadius:10,fontSize:15,fontWeight:700,cursor:'pointer',opacity:loading?0.7:1,fontFamily:'inherit'}}>
